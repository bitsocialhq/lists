...
- `seedit-directories/seedit-<code>-directory.json`: versioned candidates, discovery tags, and baseline moderation rules for each contested Seedit short route. `/s/<code>` recommends the highest-rated finalized candidate, but subscriptions and post permalinks always use exact community addresses.
...
- `5chan-search-providers.json`: indexers competing to power 5chan's `/search/`. 5chan ranks them the same way it ranks directory boards and searches with the highest-ranked reachable one, rotating to the next when an indexer cannot answer; readers can also pin one from `/search/directory`. Listing requires serving the 5chan search API (`/api/search` and `/api/posts/<cid>`) over HTTPS with CORS open to 5chan clients, since the client renders posts from the signed payload it returns. The filename intentionally does not match `5chan-<code>-directory.json`, so 5chan directory consumers ignore it.
